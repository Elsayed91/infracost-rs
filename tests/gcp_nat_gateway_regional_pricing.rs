//! Integration tests for GCP NAT Gateway regional pricing.
//!
...
//! Run with:
//! ```bash
//! cargo test --test gcp_nat_gateway_regional_pricing -- --ignored
//! ```
...
async fn test_nat_gateway_hourly_across_regions() -> Result<(), Box<dyn std::error::Error>> {
    let client = get_client()?;

    for region in TEST_REGIONS {
        let result = client.gcp().nat_gateway().region(*region).fetch().await?;

        // Validate source and price
        assert_eq!(
            result.source,
            PriceSource::Api,
            "NAT Gateway hourly in {} should use API source",
            region
        );

        assert!(
            result.price > 0.0,
            "NAT Gateway hourly price should be positive in {}",
            region
        );
        ...
        println!(
            "✓ NAT Gateway (hourly) {}: ${}/hour (source: {:?})",
...
#[tokio::test]
#[ignore = "Requires API key"]
async fn test_nat_gateway_convenience_vs_raw_hourly() -> Result<(), Box<dyn std::error::Error>> {
    let client = get_client()?;
    let region = "us-central1";

    // Test using convenience function
    let convenience_result = client.gcp().nat_gateway().region(region).fetch().await?;
    ...
    assert!(
        !products.is_empty(),
        "Raw query should return products for NAT Gateway in {}",
        region
    );

    // Filter for NAT Gateway Uptime
    ...
    assert!(
        ...
        "Should find NAT Gateway Uptime product"
    );

    let raw_price = matching_product.unwrap().first_nonzero_price_or(0.0014);
    ...
    println!(
        "✓ NAT Gateway hourly convenience vs raw in {}: ${}/hour (both API)",
...
async fn test_data_processing_price_across_regions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for region in test_regions {
        ...
        // Filter for NAT Gateway Data processing
        ...
        let price = data_product.unwrap().first_nonzero_price_or(0.045);
        ...
        println!("✓ NAT Gateway (data processing) {}: ${}/GB", region, price);
    }
...
async fn test_fetch_monthly_calculation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hourly_result = client.gcp().nat_gateway().region(region).fetch().await?;
...
async fn test_source_tracking_hourly() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        result.source,
        PriceSource::Api,
        "NAT Gateway pricing should come from API in {}",
        region
    );
...
async fn test_source_tracking_monthly() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        result.source,
        PriceSource::Api,
        "NAT Gateway monthly pricing should come from API in {}",
        region
    );
