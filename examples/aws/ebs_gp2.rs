//! AWS EBS gp2 pricing (storage only — burstable IOPS)
//!
//! ```bash
//! INFRACOST_API_KEY=xxx cargo run --example aws_ebs_gp2
//! ```
...
async fn main() -> infracost_rs::Result<()> {
    ...
    println!("=== AWS EBS gp2 (burstable IOPS) ===\n");

    println!("Unit prices:");
    for (region, geo) in &regions {
        let r = client.aws().ebs("gp2").region(*region).fetch().await?;
