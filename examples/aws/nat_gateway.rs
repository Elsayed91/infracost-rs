//! AWS NAT Gateway pricing (uptime + data processing)
//!
//! ```bash
//! INFRACOST_API_KEY=xxx cargo run --example aws_nat_gateway
//! ```
...
async fn main() -> infracost_rs::Result<()> {
    ...
    println!("=== AWS NAT Gateway ===\n");

    println!("Hourly uptime price:");
    for (region, geo) in &regions {
        let r = client.aws().nat_gateway().region(*region).fetch().await?;
