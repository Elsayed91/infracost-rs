//! AWS EBS sc1 (Cold HDD) pricing
//!
//! ```bash
//! INFRACOST_API_KEY=xxx cargo run --example aws_ebs_sc1
//! ```
...
async fn main() -> infracost_rs::Result<()> {
    ...
    println!("=== AWS EBS sc1 (Cold HDD) ===\n");

    println!("Unit prices:");
    for (region, geo) in &regions {
        let r = client.aws().ebs("sc1").region(*region).fetch().await?;
