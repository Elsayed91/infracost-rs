//! GCP Hyperdisk ML pricing (storage + throughput)
//!
//! ```bash
//! INFRACOST_API_KEY=xxx cargo run --example gcp_hyperdisk_ml
//! ```
...
async fn main() -> infracost_rs::Result<()> {
    ...
    println!("=== GCP Hyperdisk ML (storage + throughput) ===\n");

    println!("Unit prices:");
    for (region, geo) in &regions {
        let r = client
            .gcp()
            .disk(DiskType::HyperdiskMl)
            .region(*region)
            .fetch()
            .await?;
    ...
    println!("\nMonthly — 100 GB + throughput:");
    for throughput in [100, 200, 500] {
